        ...
        function executeSubmit() {
            with(document.myForm) {
                ...
                if (inputYear.value == 0) {
                    inputYear.focus();
                    jQuery("#inputYear").addClass("formInputContantTbAlertY");
                    return false;
                } else {
                    jQuery("#inputYear").removeClass("formInputContantTbAlertY");
                }
                <?php } ?>
        ...
        <div class="divRightMain">
            ...
            <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxViewBorder ">
                ...
                <?php if(!in_array($_REQUEST['masterkey'], $arr_masterkey_year)){ ?>
                    ...
                    <td width="82%" colspan="6" align="left" valign="top" class="formRightContantTb">
                        <select name="inputYear" id="inputYear" class="formSelectContantTb">
                            ...
                            $sql_subgroup .= "  FROM " . $mod_tb_root_sub . " WHERE  " . $mod_tb_root_sub . "_masterkey ='" . $_REQUEST['masterkey'] . "'  ORDER BY " . $mod_tb_root_sub . "_order DESC ";
                            $query_group = wewebQueryDB($coreLanguageSQL, $sql_subgroup);
                            while ($row_group = wewebFetchArrayDB($coreLanguageSQL, $query_group)) {
                                $row_subgroupid = $row_group[0];
                                $row_subgroupname = $row_group[1];
                            ?>
                                <option value="<?php echo $row_subgroupid ?>" <?php if ($_REQUEST['inputGh'] == $row_subgroupid) { ?> selected="selected" <?php } ?>><?php echo $row_subgroupname ?></option>
                            <?php } ?>
                        </select>
                    </td>
                </tr>
                <!-- <tr>
                    ...
                    <td width="82%" colspan="6" align="left" valign="top" class="formRightContantTb">
                        <select name="inputYear" id="inputYear" class="formSelectContantTb">
                            ...
                            <?php
                            $year_current = date('Y');
                            $year_start = $year_current - 5;
                            $year_end = $year_current + 5;
                            while ($year_end >= $year_start) { ?>
                                <option value="<?php echo $year_end ?>" <?php if ($_REQUEST['inputGh'] == $year_end) { ?> selected="selected" <?php } ?>><?php echo $year_end ?></option>
                            <?php
                                $year_end--;
                            }
                            ?>
