...
<?php
	...
	if(!empty($_FILES['inputFileUploadD']['error'])){
		...
		$error = 'No file was uploaded..';
			...
			$filenamedoc = "filed-".$_REQUEST['myID']."-".$_REQUEST['langt']."-".$myNewRand.".$fileTypeName";
...
$sqlSelect = "".$mod_tb_fileTemp."_id,".$mod_tb_fileTemp."_filename,".$mod_tb_fileTemp."_name,".$mod_tb_fileTemp."_download";
	...
	$valLimitTop ="";
	$valLimitMysql =" LIMIT 0,1";
	}

		 $sql="SELECT ".$valLimitTop." ".$sqlSelect."   FROM ".$mod_tb_fileTemp." WHERE ".$mod_tb_fileTemp."_contantid 	='".$_REQUEST['myID']."'  AND   ".$mod_tb_fileTemp."_language ='".$_REQUEST['langt']."'  ORDER BY ".$mod_tb_fileTemp."_id ASC   ".$valLimitMysql."";
			$query_file=wewebQueryDB($coreLanguageSQL,$sql);
			$number_row=wewebNumRowsDB($coreLanguageSQL,$query_file);
			//$msg .= $_REQUEST['myID'];
			if($number_row>=1){
