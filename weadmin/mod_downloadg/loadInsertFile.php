...
<?php
	...
	if(!empty($_FILES['inputFileUpload']['error'])){
		...
		$error = 'No file was uploaded..';
			...
			if(file_exists($mod_path_file."/".$_REQUEST['nametodoc'])) {
				@unlink($mod_path_file."/".$_REQUEST['nametodoc']);
			}

			$inputFName=$_FILES['inputFileUpload']['name'];
			$fType=explode(".",$inputFName);
			$cType = count($fType)-1;
			$tt=$fType[$cType];
			if($tt=='docx' || $tt=='doc' || $tt=='xls' || $tt=='xlsx' || $tt=='pdf'){
		...
		///	if($fileType=='docx' || $fileType=='xls' || $fileType=='pdf'){
			...
			$msg.= '<font color =red><b>Plase upload only .pdf, .docx, .xls</b></font>';
		}
