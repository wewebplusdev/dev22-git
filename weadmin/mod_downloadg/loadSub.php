...
<body>
  ...
  if ($_REQUEST['module_orderby'] == "") {
    $module_orderby = $mod_tb_root_sub . "_order";
  } else {
  ...
  if ($_REQUEST['inputGh'] >= 1) {
    $sqlSearch = $sqlSearch . "  AND " . $mod_tb_root_sub . "_gid ='" . $_REQUEST['inputGh'] . "'   ";
  }
  ...
  if ($inputSearch <> "") {
    $sqlSearch = $sqlSearch . "  AND  (
		" . $mod_tb_root_sub . "_subject LIKE '%$inputSearch%'  OR
		" . $mod_tb_root_sub . "_subjecten LIKE '%$inputSearch%'   ) ";
  }
        ...
        <tr>
          ...
          <td class="divRightNavTb" align="right">
            ...
            <div class="menuSubMod active">
              <a href="sub.php?masterkey=<?php echo  $_REQUEST['masterkey'] ?>&menukeyid=<?php echo  $_REQUEST['menukeyid'] ?>">
                <?php echo  $langMod["meu:sub"] ?>
              </a>
            </div>
        ...
        <tr>
          <!-- <td style="padding-right:10px;" width="42%">
            <select name="inputGh" id="inputGh" onchange="document.myForm.submit(); " class="formSelectSearchStyle">
              <option value="0"><?php echo  $langMod["op:selectg"] ?> </option>
              <?php
              ...
              <tr>
                <td align="right">
                  <?php if ($valPermission == "RW") { ?>
                    <div class="btnAdd" title="<?php echo  $langTxt["btn:add"] ?>" onclick="document.myFormHome.inputLt.value='Thai';  addContactNew('addSub.php');"></div>
...
if(Paging_CountChecked('CheckBoxID',document.myForm.TotalCheckBoxID.value)>0) {
	if(confirm('<?php echo  $langTxt["mg:delpermis"] ?>')) {
          delContactNew('deleteSub.php');
	}
} else {
		alert('<?php echo  $langTxt["mg:selpermis"] ?>');
}
				  "></div> <?php ?>
                    <div class="btnSort" title="<?php echo  $langTxt["btn:sortting"] ?>" onclick="sortContactNew('sortSub.php');"></div>
                <?php  } ?>
                </td>
              </tr>
            </table>
    ...
    <div class="divRightMain">
      <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxListwBorder">
        <tr>
          ...
          <td align="left" valign="middle" class="divRightTitleTb"><span class="fontTitlTbRight"><?php echo  $langMod["tit:subjectsg"] ?><?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2) { ?>(<?php echo  $langTxt["lg:thai"] ?>)<?php } ?></span></td>
          <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2) { ?>
            <td width="30%" align="left" valign="middle" class="divRightTitleTb"><span class="fontTitlTbRight"><?php echo  $langMod["tit:subjectsg"] ?><?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2) { ?>(<?php echo  $langTxt["lg:eng"] ?>)<?php } ?></span></td>
          <?php } ?>
        ...
        $sqlSelect = "
" . $mod_tb_root_sub . "_id,
" . $mod_tb_root_sub . "_subject,
" . $mod_tb_root_sub . "_lastdate,
" . $mod_tb_root_sub . "_status,
" . $mod_tb_root_sub . "_subjecten,
" . $mod_tb_root_sub . "_pic
";

        // SQL SELECT #########################
        $sql = "SELECT  " . $sqlSelect . "   FROM " . $mod_tb_root_sub;
        $sql = $sql . "  WHERE " . $mod_tb_root_sub . "_masterkey ='" . $_REQUEST['masterkey'] . "'   ";
        $sql = $sql . $sqlSearch;
        ...
        if ($coreLanguageSQL == "mssql") {
          $sql = "SELECT " . $sqlSelect . " FROM (SELECT     RuningCount = ROW_NUMBER() OVER (ORDER BY " . $module_orderby . "  " . $module_adesc . " ),*  FROM   " . $mod_tb_root_sub;
          $sql .= "  WHERE " . $mod_tb_root_sub . "_masterkey ='" . $_REQUEST['masterkey'] . "'   ";
        ...
        if ($count_record > 0) {
          while ($index < $count_record + 1) {
            ...
            $valView = number_format($valView1 + $valView2);
        ?>
   ...
   document.myFormHome.valEditID.value=<?php echo  $valID ?>;
    viewContactNew('viewSub.php');"><?php echo  $valName ?></a></div>
                    </td>
                  </tr>
                </table>
              </td>
   ...
   document.myFormHome.valEditID.value=<?php echo  $valID ?>;
    viewContactNew('viewSub.php');"><?php echo  $valNameEn ?></a></div>
                      </td>
                    </tr>
                  </table>
                </td>
                ...
                <?php if ($valPermission == "RW") { ?>
                  <div id="load_status<?php echo  $valID ?>">
                    <?php if ($valStatus == "Enable") { ?>
                      ...
                      <a href="javascript:void(0)" onclick="changeStatus('load_waiting<?php echo  $valID ?>','<?php echo  $mod_tb_root_sub ?>','<?php echo  $valStatus ?>','<?php echo  $valID ?>','load_status<?php echo  $valID ?>','../<?php echo  $mod_fd_root ?>/statusMg.php')"><span class="<?php echo  $valStatusClass ?>"><?php echo  $valStatus ?></span></a>
   ...
   document.myFormHome.inputLt.value='Thai';
   document.myFormHome.valEditID.value=<?php echo  $valID ?>;
    editContactNew('topUpdateSub.php');">
