...
<body>
  ...
  $contant_orderby = $mod_tb_root . "_order";
  ...
  $sqlSearch = "";
  if ($inputSearch <> "") {
    $sqlSearch = $sqlSearch . "  AND   ( " . $mod_tb_root . "_subject LIKE '%$inputSearch%'  ) ";
  }

  if ($inputGh >= 1) {
    $sqlSearch = $sqlSearch . "  AND    " . $mod_tb_root . "_groupProoject='" . $inputGh . "'  ";
  }
  ...
  $sql_export = $sql_export . "  WHERE   " . $mod_tb_root . "_masterkey='" . $_POST["masterkey"] . "' AND  " . $mod_tb_root . "_language  ='" . $_SESSION['dmcr_core_session_language'] . "' ";
...
if(Paging_CountChecked('CheckBoxID',document.myForm.TotalCheckBoxID.value)>0) {
	if(confirm('<?php echo $langTxt["mg:delpermis"] ?>')) {
		...
		alert('<?php echo $langTxt["mg:selpermis"]  ?>');
				  ...
				  "></div>
                    <?php if ($_REQUEST['inputGh'] != 0 || true) {  ?>
...
" . $mod_tb_root . "_subjecten as subjecten,
" . $mod_tb_root . "_urlen as urlen,
" . $mod_tb_root . "_subjectcn as subjectcn,
" . $mod_tb_root . "_urlcn as urlcn
...
FROM " . $mod_tb_root;
        ...
        if ($_REQUEST['inputGh'] >= 1) {
          $sql = $sql . "  AND " . $mod_tb_root . "_groupProoject ='" . $_REQUEST['inputGh'] . "'   ";
        }
...
$sql = $sql."  AND ".$mod_tb_root."_tid ='".$_REQUEST['inputTh']."'   ";
        ...
        if ($count_record > 0) {
          while ($index < $count_record + 1) {
            ...
            $valUrl = $row['url'];
            $valUrlen = $row['urlen'];
            $valNameEn = rechangeQuot($row['subjecten']);
