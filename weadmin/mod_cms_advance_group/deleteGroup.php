...
for ($i = 1; $i <= $_REQUEST['TotalCheckBoxID']; $i++) {
	...
	if (strlen($myVar) >= 1) {
		$permissionID = $myVar;
		$sql = "SELECT  " . $mod_tb_root_group . "_pic, " . $mod_tb_root_group . "_htmlfilename, " . $mod_tb_root_group . "_htmlfilenameen, " . $mod_tb_root_group . "_htmlfilenamecn FROM " . $mod_tb_root_group . " WHERE  " . $mod_tb_root_group . "_id='" . $permissionID . "' ";
		$Query = wewebQueryDB($coreLanguageSQL, $sql);
		$Row = wewebFetchArrayDB($coreLanguageSQL, $Query);
		$deletepic = $Row[0];
		$deletepichtml = $Row[1];
		$deletepichtmlen = $Row[2];
		$deletepichtmlcn = $Row[3];
