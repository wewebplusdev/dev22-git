...
<?php
	...
	if(!empty($_FILES['inputVideoUpload']['error'])){
		...
		$error = 'No file was uploaded..';
			...
			if(   $fileTypeName=='mp4' || $fileTypeName =="MP4"   ){
				// || $fileTypeName=='flv' || $fileTypeName =="FLV" || $fileTypeName=='wmv' || $fileTypeName =="WMV" ||
			...
			$filenamedoc = "vdo-".$myNewRand.".$fileTypeName";
			...
			// if (get_IconSize($linkRelativePath) <= "100.00 Mb") {
			if ($SizeVideo <= 100.00) { 
				...
				$msg.="<font color=red><b>กรุณาตรวจสอบขนาดไฟล์ ไฟล์อัพโหลดจะต้องขนาดไม่เกิน 100 Mb เท่านั้น</b></font>";
			}
		 	// print_pre(get_IconSize($linkRelativePath));
		}else{
			$msg.="<font color=red><b>กรุณาตรวจสอบไฟล์ อัพโหลดเฉพาะไฟล์ .flv, .wmv, .mp3,.mp4, .wav, .wma, .avi และ .mpeg เท่านั้น</b></font>";
		}
	}		
	echo "{";
