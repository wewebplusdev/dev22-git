...
for($i=1;$i<=$_REQUEST['TotalCheckBoxID'];$i++) {
 $myVar=$_REQUEST['CheckBoxID'.$i];
	...
	$permissionID=$myVar;
			...
			$sql = "SELECT  ".$mod_tb_root."_htmlfilename,".$mod_tb_root."_pic,".$mod_tb_root."_filevdo,".$mod_tb_root."_htmlfilenameen,".$mod_tb_root."_picen,".$mod_tb_root."_piccn FROM ".$mod_tb_root." WHERE  ".$mod_tb_root."_id='".$permissionID."' ";
			...
			if(file_exists($mod_path_pictures."/".$deletepiccn)) {
				@unlink($mod_path_pictures."/".$deletepiccn);
			}
			
			if(file_exists($mod_path_office."/".$deletepiccn)) {
				@unlink($mod_path_office."/".$deletepiccn);
			}
