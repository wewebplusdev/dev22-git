    ...
    <form action="?" method="post" name="myForm" id="myForm">
        ...
        <div class="divRightHeadSearch">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-top:20px;" align="center">
                <tr>
                    <td width="48%">
                        <select name="inputG3" id="inputG3" onchange="document.myForm.submit(); " class="formSelectSearchStyle">
                            <option value="0"><?php echo  $langMod["tit:selectstatus"] ?> </option>
                            <option value="Enable" <?php if ($_REQUEST['inputG3'] == 'Enable') { ?> selected="selected" <?php  } ?>>Enable</option>
                            <option value="Disable" <?php if ($_REQUEST['inputG3'] == 'Disable') { ?> selected="selected" <?php  } ?>>Disable</option>
                        </select>
                    </td>
                    <td id="boxSelectTest" width="50%">
                        <!-- <input name="inputSearch" type="text"  id="inputSearch" value="<?php echo  trim($_REQUEST['inputSearch']) ?>" class="formInputSearch"  placeholder="<?php echo  $langTxt["sch:search"] ?>" /> -->
        ...
        <div class="divRightMain">
            ...
            if (isset($inputSearch) && !empty($inputSearch)) {
                $WhereSearch = " AND (" . $mod_tb_root . "." . $mod_tb_root . "_subject LIKE '%$inputSearch%' OR " . $mod_tb_root . "." . $mod_tb_root . "_subject LIKE '%$inputSearch%')";
            }
            $sql    = " SELECT
                                ...
                                " . $mod_tb_root . "." . $mod_tb_root . "_subjecten as subjecten,
                                " . $mod_tb_root . "." . $mod_tb_root . "_subjectcn as subjectcn
                            FROM " . $mod_tb_root . "
                            WHERE 1=1
                            AND " . $mod_tb_root . "." . $mod_tb_root . "_masterkey = '" . $_REQUEST['masterkey'] . "'
                            " . $WhereSearch . "
                ";
            if (!empty($_REQUEST['inputG3'])) {
                $sql .= " AND " . $mod_tb_root . "." . $mod_tb_root . "_status = '" . $_REQUEST['inputG3'] . "'";
            }
            ...
            <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxListwBorder">
                <tr>
                    ...
                    <td width="30%" align="left" valign="middle" class="divRightTitleTb">
                        <span class="fontTitlTbRight">
                            <?php echo $langMod["tit:inpName"] ?>
                    ...
                    <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] >= 2) : ?>
                        <td width="30%" align="left" valign="middle" class="divRightTitleTb">
                            <span class="fontTitlTbRight">
                                <?php echo $langMod["tit:inpName"] ?>
                                <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] >= 2) { ?>(<?php echo $langTxt["lg:chi"] ?>)
                            <?php } ?>
                            </span>
                        </td>
                    <?php endif; ?>
                ...
                <?php if ($count_record > 0) : ?>
                    <?php foreach ($query as $key => $value) : ?>
                        <?php
                        $valID = $value[0];
                        $valName = rechangeQuot($value[1]);
                        $valDateCredate = dateFormatReal($value[2]);
                        $valTimeCredate = timeFormatReal($value[2]);
                        $valStatus = $value[3];
                        $valNameEn = rechangeQuot($value['subjecten']);
                        $valNameEn = chechNullVal($valNameEn);
                        $valPic = $mod_path_office . "/" . $value[4];
                        $valNameCn = rechangeQuot($value['subjectcn']);
                        ...
                        <tr class="<?php echo $valDivTr; ?>">
                            <td class="divRightContantOverTbL" valign="top" align="center">
                                <input id="CheckBoxID<?php echo $index ?>" name="CheckBoxID<?php echo $index ?>" type="checkbox" class="formCheckboxList" onClick="Paging_CheckAllHandle(document.myForm.CheckBoxAll, 'CheckBoxID', document.myForm.TotalCheckBoxID.value)" value="<?php echo $valID ?>" />
                            </td>
                                ...
                                <?php if ($valPermission == "RW") : ?>
                                    <div id="load_status<?php echo $valID ?>">
                                        <?php if ($valStatus == "Enable") { ?>
                                            <a href="javascript:void(0)" onclick="changeStatus('load_waiting<?php echo $valID ?>', '<?php echo $mod_tb_root ?>', '<?php echo $valStatus ?>', '<?php echo $valID ?>', 'load_status<?php echo $valID ?>', '../<?php echo $mod_fd_root ?>/statusMg.php')">
                                        ...
                                        <tr>
                                            <td valign="top" align="center" width="30">
                                                <div class="divRightManage" title="<?php echo $langTxt["btn:top"] ?>" onclick="
                                                    ...
                                                    <span class="fontContantTbManage"><?php echo $langTxt["btn:top"] ?><br>
                                            ...
                                            <td valign="top" align="center" width="30">
                                                <div class="divRightManage" title="<?php echo $langTxt["btn:edit"] ?>" onclick="
                                                    ...
                                                    <span class="fontContantTbManage"><?php echo $langTxt["btn:edit"] ?>
                                                        <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2  || $_SESSION[$valSiteManage . 'core_session_languageT'] > 2) { ?><br />
                                                            (<?php echo $langTxt["lg:thai"] ?>)
                                                        <?php } ?>
                                                    </span>
                                                </div>
                                            </td>
                                            <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2 || $_SESSION[$valSiteManage . 'core_session_languageT'] > 2) : ?>
                                                <td valign="top" align="center" width="30">
                                                    <div class="divRightManage" title="<?php echo $langTxt["btn:edit"] ?>" onclick="
                                                        document.myFormHome.inputLt.value = 'Eng';
                                                        document.myFormHome.valEditID.value =<?php echo $valID ?>;
                                                        editContactNew('editContant.php');">
                                                        <img src="../img/btn/edit.png" /><br />
                                                        <span class="fontContantTbManage"><?php echo $langTxt["btn:edit"] ?><br />(<?php echo $langTxt["lg:eng"] ?>)</span>
                                                    </div>
                                                </td>
                                            <?php endif; ?>
                                            <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2 || $_SESSION[$valSiteManage . 'core_session_languageT'] > 2) : ?>
                                                <td valign="top" align="center" width="30">
                                                    <div class="divRightManage" title="<?php echo $langTxt["btn:edit"] ?>" onclick="
                                                        ...
                                                        editContactNew('editContant.php');">
                                                        <img src="../img/btn/edit.png" /><br />
                                                        <span class="fontContantTbManage"><?php echo $langTxt["btn:edit"] ?><br />(<?php echo $langTxt["lg:chi"] ?>)</span>
                                                    </div>
                                                </td>
                                            <?php endif; ?>
                                            <td valign="top" align="center" width="30">
                                                ...
                                                ">
                                                    ...
                                                    <span class="fontContantTbManage">
                                                        <?php echo $langTxt["btn:del"] ?>
                                                        <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2 || $_SESSION[$valSiteManage . 'core_session_languageT'] > 2) { ?><br />
                                                            (<?php echo $langTxt["lg:all"] ?>)
                                                        <?php } ?>
                                                    </span>
