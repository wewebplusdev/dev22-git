...
$sql = "SELECT " . $mod_tb_root_email . "_email 
		FROM " . $mod_tb_root_email . "  
		WHERE  " . $mod_tb_root_email . "_masterkey='" . $_REQUEST["masterkey"] . "'  
		AND   " . $mod_tb_root_email . "_email='" . $_REQUEST['inputEmail'] . "' 
		";
if ($_REQUEST['action'] == 'group') {
	$sql .= "
			AND   " . $mod_tb_root_email . "_gid='" . $_REQUEST['valEditID'] . "'
			";
} else {
	$sql .= "
...
if ($count_record <= 0) {
	...
	// $insert[$mod_tb_root_email . "_action"] = "'" . changeQuot($_REQUEST['action']) . "'";
	if ($_REQUEST['action'] == 'group') {
		$insert[$mod_tb_root_email . "_gid"] = "'" . changeQuot($_REQUEST['valEditID']) . "'";
	} else {
		$insert[$mod_tb_root_email . "_sid"] = "'" . changeQuot($_REQUEST['valEditID']) . "'";
	}
...
WHERE  " . $mod_tb_root_email . "_masterkey='" . $_REQUEST["masterkey"] . "' ";
	if ($_REQUEST['action'] == 'group') {
		$sql .= " AND   " . $mod_tb_root_email . "_gid='" . $_REQUEST['valEditID'] . "' ";
	} else {
		$sql .= " AND   " . $mod_tb_root_email . "_sid='" . $_REQUEST['valEditID'] . "' ";
	}
	$sql .= "
