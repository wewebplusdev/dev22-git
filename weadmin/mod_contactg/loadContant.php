		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-top:20px;" align="center">
				...
				<td style="padding-right:10px;" width="30%">
						<select name="inputGh" id="inputGh" onchange="document.myForm.submit(); " class="formSelectSearchStyle">
							<option value="0"><?php echo $langMod["tit:subjectsg"] ?> </option>
							<?php
...
if(Paging_CountChecked('CheckBoxID',document.myForm.TotalCheckBoxID.value)>0) {
	if(confirm('<?php echo $langTxt["mg:delpermis"] ?>')) {
		...
		<div class="divRightMain">
			<!-- comment 2 แสดงจำนวนวัน และ สิทธิ -->
			<table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxListwBorder">
				<tr>
					...
					<td align="left" valign="middle" class="divRightTitleTb" width="30%"><span class="fontTitlTbRight"><?php echo $langMod["tit:subject"] ?></span></td>

					<!-- <td width="20%"  class="divRightTitleTb"  valign="middle"  align="center"><span class="fontTitlTbRight"><?php echo $langMod["tit:qty"] ?></span></td> -->
					<!-- menuba -->
					<td width="15%" class="divRightTitleTb" valign="middle" align="left"><span class="fontTitlTbRight"><?php echo $langMod["tit:subjectsg"]; ?></span></td>
					<!-- <td width="15%"  class="divRightTitleTb"  valign="middle"  align="center"><span class="fontTitlTbRight">สถานะสิทธิ์</span></td> -->
					<td width="12%" class="divRightTitleTb" valign="middle" align="center"><span class="fontTitlTbRight">จำนวนวัน</span></td>
	...
	FROM " . $mod_tb_root . "  WHERE " . $mod_tb_root . "_delete ='Enable'";
				...
				if ($count_record > 0) {
					while ($index < $count_record + 1) {
						...
						$valaclaim = unserialize($row[13]);
						$valaEnable = rechangeQuot($row[14]);
						...
						/* $mysql = "SELECT ".$mod_tb_root_subgroup."_id FROM ".$mod_tb_root_subgroup." WHERE ".$mod_tb_root_subgroup."_id = VALUES (".implode(",",array_values($insert)).") ";
		print_r($mysql) */
						/* 	$mysql = "SELECT
		".$mod_tb_root."_id, 
		".$mod_tb_root."_sid,
		...
		FROM ".$mod_tb_root." INNER JOIN ".$mod_tb_root_subgroup."ON ".$mod_tb_root."_gid = ".$mod_tb_root_subgroup."_id WHERE _id IN (".implode(",",array_values($insert)).")";
	$mysql = $mysql."  WHERE ".$mod_tb_root."_masterkey ='".$_REQUEST['masterkey']."'   ";
print_r($mysql); */
   ...
   document.myFormHome.valEditID.value=<?php echo $valID ?>;  
    viewContactNew('viewContant.php');"><?php echo $langMod["tit:by"] ?> : <?php echo $valName ?></a>
							...
							<!-- table numdate -->
							<td align="left" valign="top" class="divRightContantOverTb"><span>
									<?php
									foreach ($claim as $value) {
										echo "- $value<br>";
									}
									?></td>
							<!-- <td align="center"   valign="top"  class="divRightContantOverTb" ><span><?php echo $valaEnable ?></td> -->
							<td align="center" valign="top" class="divRightContantOverTb"><span><?php echo $datealltime ?></td>
          ...
          delContactNew('deleteContant.php');
            ...
            ">
										...
										<!-- end comment 2 แสดงจำนวนวัน และ สิทธิ -->
									</table>
								<?php } ?>
							</td>
						</tr>
