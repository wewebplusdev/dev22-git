...
<body>
	...
	if (!empty($_FILES['inputFileUpload']['error'])) {
		...
		$sql_fileNew="SELECT ".$mod_tb_fileTemp."_filename  FROM ".$mod_tb_fileTemp." WHERE ".$mod_tb_fileTemp."_contantid 	='".$_REQUEST['myID']."' ";
		...
		$sql="DELETE FROM ".$mod_tb_fileTemp." WHERE   ".$mod_tb_fileTemp."_contantid='".$_REQUEST['myID']."' ";
		...
		$sql = "SELECT " . $mod_tb_fileTemp . "_id," . $mod_tb_fileTemp . "_filename," . $mod_tb_fileTemp . "_name FROM " . $mod_tb_fileTemp . " WHERE " . $mod_tb_fileTemp . "_contantid 	='" . $_REQUEST['myID'] . "'   ORDER BY " . $mod_tb_fileTemp . "_id ASC";
		...
		if ($number_row >= 1) {
			while ($row_file = wewebFetchArrayDB($coreLanguageSQL, $query_file)) {
				...
				$msg .= "<a href=\"javascript:void(0)\"  onclick=\" document.myForm.valDelFile.value=" . $downloadID . ";delFileUpload(\'deleteFileInsert.php\')\" ><img src=\"../img/btn/delete.png\" align=\"absmiddle\" title=\"Delete file\"  hspace=\"10\"  vspace=\"10\"   border=\"0\" /></a>" . $downloadName . " | " . $langMod["file:type"] . ": " . $imageType . "  | " . $langMod["file:size"] . ": " . get_IconSize($linkRelativePath) . " <br/>";
			}
		}
	}
	echo "{";
