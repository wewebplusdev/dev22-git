...
<?php
	...
	if(!empty($_FILES['fileToUploadMap']['error'])){
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			$TIME=time();
			if(!$ERROR) {
			$myNewRand = rand(11,99);
			$filename="pic-m-".$_REQUEST['myID']."-".$myNewRand;
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUploadMap(\'deletePicInsertMap.php\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
