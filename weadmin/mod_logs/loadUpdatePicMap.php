...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadMap';
	if(!empty($_FILES['fileToUploadMap']['error'])){
		switch($_FILES['fileToUploadMap']['error']){
	...
	}elseif($_FILES['fileToUploadMap']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			$inputGallery=$_FILES['fileToUploadMap']['tmp_name'];
			$arrfile=$_FILES['fileToUploadMap'];
			$ERROR=$_FILES['fileToUploadMap']['error'];
			$TIME=time();
			if(!$ERROR) {
			$myRandNew =$coreFileDateTime;
			$filename="pic-m-".$_REQUEST['myID']."-".$myRandNew;
			...
			if($arrImgInfo[0]<=($sizeWidthMap+10)){
		...
		$update = array();
		$update[]=$mod_tb_root."_picmap  	='".$picname."'";
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUploadMap(\'deletePicMap.php\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnameMap\" type=\"hidden\" id=\"picnameMap\" value=\"$picname\" /> ";
