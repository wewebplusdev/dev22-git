...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadQrcode';
	if(!empty($_FILES['fileToUploadQrcode']['error'])){
		switch($_FILES['fileToUploadQrcode']['error']){
	...
	}elseif($_FILES['fileToUploadQrcode']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			$inputGallery=$_FILES['fileToUploadQrcode']['tmp_name'];
			$arrfile=$_FILES['fileToUploadQrcode'];
			$ERROR=$_FILES['fileToUploadQrcode']['error'];
			$TIME=time();
			if(!$ERROR) {
			$myRandNew =$coreFileDateTime;
			$filename="pic-q-".$_REQUEST['myID']."-".$myRandNew;
			...
			if($arrImgInfo[0]<=($sizeWidthQrcode+10)){
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUploadQrcode(\'deletePicQrcode.php\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnameQrcode\" type=\"hidden\" id=\"picnameQrcode\" value=\"$picname\" /> ";
