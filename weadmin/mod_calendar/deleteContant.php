...
for($i=1;$i<=$_REQUEST['TotalCheckBoxID'];$i++) {
 $myVar=$_REQUEST['CheckBoxID'.$i];
	...
	$permissionID=$myVar;
			...
			$sql = "SELECT  ".$mod_tb_root."_htmlfilename,".$mod_tb_root."_pic,".$mod_tb_root."_filevdo,".$mod_tb_root."_map FROM ".$mod_tb_root." WHERE  ".$mod_tb_root."_id='".$permissionID."' ";
			...
			######################### Delete  In Folder Map ###############################
			if(file_exists($mod_path_pictures."/".$deletemap)) {
				@unlink($mod_path_pictures."/".$deletemap);
			}
			
			
			if(file_exists($mod_path_real."/".$deletemap)) {
				@unlink($mod_path_real."/".$deletemap);
			}
			...
			######################### Delete  Contant ###############################
			$sqlDate="DELETE FROM ".$mod_tb_date." WHERE ".$mod_tb_date."_cid=".$permissionID." ";
...
<form action="index.php" method="post" name="myFormAction" id="myFormAction">
    ...
    <input name="sdateInputSe" type="hidden" id="sdateInputSe" value="<?php echo $_REQUEST['sdateInputH']?>" />
    <input name="edateInputSe" type="hidden" id="edateInputSe" value="<?php echo $_REQUEST['edateInputH']?>" />
