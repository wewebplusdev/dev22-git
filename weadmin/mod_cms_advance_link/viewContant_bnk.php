...
if ($_REQUEST['inputLt'] == "Thai") {
    ...
    $sql .= " ,
     ...
     " . $mod_tb_root . "_htmlfilenamecn ,    
     " . $mod_tb_root . "_metatitlecn  ,    
     " . $mod_tb_root . "_descriptioncn ,    
     " . $mod_tb_root . "_keywordscn    ";
}
$sql .= " , 
" . $mod_tb_root . "_sgid, " . $mod_tb_root . "_pic   ";
...
$valGid = $Row[11];
$fileUp = $Row[12];
$valSubject = rechangeQuot($Row[13]);
$valTitle = rechangeQuot($Row[14]);
$valSgid = $Row[15];
$valPic=$mod_path_real."/".$Row[16];
                    ...
                    <form action="?" method="get" name="myForm" id="myForm">
                        ...
                        <? if ($_REQUEST['viewID'] <= 0) { ?>
                            <div class="divRightNav">
                                <table width="96%" border="0" cellspacing="0" cellpadding="0"  align="center" >
                                    <tr>
                                        <td  class="divRightNavTb" align="left"  id="defTop" ><span class="fontContantTbNav"><a href="<?= $valLinkNav1 ?>" target="_self"><?= $valNav1 ?></a> <img src="../img/btn/nav.png" align="absmiddle" vspace="5" /> <a  href="javascript:void(0)"  onclick="btnBackPage('index.php')" target="_self"><?= getNameMenu($_REQUEST["menukeyid"]); ?></a> <img src="../img/btn/nav.png" align="absmiddle" vspace="5" /> <?= $langMod["txt:titleview"] ?><? if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2 || $_SESSION[$valSiteManage . 'core_session_languageT'] == 3) { ?>(<?= getSystemLangTxt($_REQUEST['inputLt'], $langTxt["lg:thai"], $langTxt["lg:eng"]) ?>)<? } ?></span></td>
                        ...
                        <div class="divRightMain" >
                            ...
                            <? }?>
                                ...
                                <?if($_REQUEST['masterkey']!='ru' && $_REQUEST['masterkey']!='op' && $_REQUEST['masterkey']!='ps' ){?>
                                ...
                                <tr >
                                    ...
                                    <td width="82%" colspan="6" align="left"  valign="top"  class="formRightContantTb" ><div class="formDivView">
                                            ...
                                            $sql_group .= "  FROM " . $mod_tb_root_sub_group . " WHERE  " . $mod_tb_root_sub_group . "_id='" . $valSgid . "'  ORDER BY " . $mod_tb_root_sub_group . "_order DESC ";
                                            $query_group = wewebQueryDB($coreLanguageSQL, $sql_group);
                                            $row_group =wewebFetchArrayDB($coreLanguageSQL, $query_group);
                                            $row_groupid = $row_group[0];
                                           $row_groupname = $row_group[1];
                                            if($row_groupname!=""){echo $row_groupname;}else{echo "-";}
                                            ?>
                                        </div></td>
                                </tr>
                                ...
                                <tr >
    <td width="18%" align="right"  valign="top"  class="formLeftContantTb" >ไฟล์อัพโหลด<?//=$langMod["tit:url"]?>:<span class="fontContantAlert"></span></td>
...
<td width="82%" colspan="6" align="left"  valign="top"  class="formRightContantTb" ><div class="formDivView">
    ...
    <a href="<?=$valUrl?>" target="_blank"><?=$valUrl?></a>  <font class="fontfile"> ประเภท : link </font>
      <? }else{
        ...
        echo "-";
            ...
            $sql="SELECT ".$mod_tb_root."_id,".$mod_tb_root."_file,".$mod_tb_root."_download,".$mod_tb_root."_filename FROM ".$mod_tb_root." WHERE ".$mod_tb_root."_id   ='".$valID."'  ORDER BY ".$mod_tb_root."_id ASC";
            // print_pre($sql);
            ...
            <a href="../<?=$mod_fd_root?>/download.php?linkPath=<?=$linkRelativePath?>&amp;downloadFile=<?=$downloadName?>">ดาวน์โหลดคลิกที่นี่<!-- <?=$downloadName.""?> --></a> | 
            <font class="fontfile"><?=$langMod["file:type"]?>: <?=$imageType?> | <?=$langMod["file:size"]?>: <?=get_IconSize($linkRelativePath)?> | <?=$langMod["file:download"]?>: <?=number_format($countDownload)?></font></div>
            <div></div>
...
?>
                            ...
                            <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxViewBorder " style="display:none;"> 
                                ...
                                <tr >
                                    <td width="18%" align="right"  valign="top"  class="formLeftContantTb" ><span class="fontContantAlert"></span></td>
                                    <td width="82%" colspan="6" align="left"  valign="top"  class="formRightContantTb" ><div class="formDivView">
                                            <img src="<?= $mod_path_pictures . "/" .$valPic ?>"  style="float:left;border:#c8c7cc solid 1px; max-width:600px;"  onerror="this.src='<?= "../img/btn/nopic.jpg" ?>'"  />
                                        </div></td>
                                </tr>
                                <tr >
...
<? }?>
    ...
    <?
            ...
            <div  style="float:left; width:100%; height:30px; margin-bottom:15px;"><img src="<?=get_Icon($downloadFile)?>" align="absmiddle" width="30"  /><a href="../<?=$mod_fd_root?>/download.php?linkPath=<?=$linkRelativePath?>&amp;downloadFile=<?=$downloadName?>"><?=$downloadName."".$imageType?></a> <font class="fontfile">| <?=$langMod["file:type"]?>: <?=$imageType?> | <?=$langMod["file:size"]?>: <?=get_IconSize($linkRelativePath)?> | <?=$langMod["file:download"]?>: <?=number_format($countDownload)?></font></div>
            <div></div>
...
?>
  ...
  <br>  <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxViewBorder " style="display:none;">
                                <tr >
                                    <td colspan="7" align="left"  valign="middle" class="formTileTxt tbBoxViewBorderBottom">
