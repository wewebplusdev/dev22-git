      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-top:20px;" align="center">
        ...
        <tr>
          <td width="50%"><select name="inputGh" id="inputGh" onchange="document.myForm.submit(); " class="formSelectSearchStyle">
              ...
              <?
              $sql_group = "SELECT " . $mod_tb_root_group . "_id," . $mod_tb_root_group . "_subject FROM " . $mod_tb_root_group . " WHERE  " . $mod_tb_root_group . "_status !='Disable' AND " . $mod_tb_root_group . "_masterkey ='" . $_REQUEST['masterkey'] . "'   ORDER BY " . $mod_tb_root_group . "_order DESC ";
          ...
          <td width="50%" <?= $display ?>><select name="inputSGh" id="inputSGh" onchange="document.myForm.submit(); " class="formSelectSearchStyle">
              ...
              ?>
                  <option value="<?= $row_groupid ?>" <? if ($_REQUEST['inputSGh'] == $row_groupid) { ?> selected="selected" <?  } ?>><?= $valNameShow ?></option>
              <? }
              } ?>
            </select></td>
        ...
        <tr>
          <td id="boxSelectTest" align="right" colspan="<?= $colspan_td ?>">
...
FROM " . $mod_tb_root;
        ...
        if ($_REQUEST['inputSGh'] >= 1) {
          $sql = $sql . "  AND " . $mod_tb_root . "_sgid ='" . $_REQUEST['inputSGh'] . "'   ";
        }
...
<a href="javascript:void(0)"  onclick="changeStatus('load_waiting<?= $valID ?>','<?= $mod_tb_root ?>','<?= $valStatus ?>','<?= $valID ?>','load_status<?= $valID ?>','../<?= $mod_fd_root ?>/statusMgH.php')" ><span class="<?= $valStatusClass ?>"><?= $valStatus ?></span></a> -->
              ...
              <!--   <td  class="divRightContantOverTb"  valign="top"  align="center">
    <select name="inputRibbon<?= $valID ?>" id="inputRibbon<?= $valID ?>" onchange="changeRibbon('<?= $mod_tb_root ?>','<?= $valRibbon ?>','<?= $valID ?>','../<?= $mod_fd_root ?>/statusMgR.php')">
     ...
     <td  class="divRightContantOverTb"  valign="top"  align="center">
       <? if ($valPermission == "RW") { ?>
     <div   id="load_pin<?= $valID ?>" class="btnstatus" data-id="<?= $valID ?>">
    <? if ($valPin == "Pin") { ?>
    ...
    editContactNew('editContant.php');">
          ...
          delContactNew('deleteContant.php');
            ...
            ">
                      ...
                      <!-- <td valign="top" align="center" width="30">
<div  class="btnSort divRightManage" title="<?= $langTxt["btn:sortting"] ?>" onclick="sortContactNew('sortContant.php');"></div>

</td> -->
                      <td valign="top" align="center" width="30">
                        <div class="divRightManage" title="<?= $langTxt["btn:sortting"] ?>" onclick="
   document.myFormHome.valEditID.value=<?= $valID ?>;
   sortContactNew('sortSub.php');">
                          <img width="22" src="../img/btn/sort.png" /><br />
                          <span class="fontContantTbManage">เรียง<? if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2) { ?><br />
                            (Sort)<? } ?></span>
                        </div>
                      </td>
                    </tr>
