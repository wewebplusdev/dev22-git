...
for ($i = 1; $i <= $_REQUEST['TotalCheckBoxID']; $i++) {
	...
	if (strlen($myVar) >= 1) {
		...
		$sql = "DELETE FROM " . $mod_tb_root_sub_group . " WHERE " . $mod_tb_root_sub_group . "_id=" . $permissionID . " ";
		$Query = wewebQueryDB($coreLanguageSQL, $sql);

		$sql = "DELETE FROM " . $mod_tb_root_sub_group . " WHERE " . $mod_tb_root_sub_group . "_parentid='" . $permissionID . "'";
		$Query = wewebQueryDB($coreLanguageSQL, $sql);
	}
}
