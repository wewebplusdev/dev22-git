...
<?php
	...
	if(!empty($_FILES['fileToUpload2']['error'])){
		...
		$error = 'No file was uploaded..';
	}elseif ($_FILES['fileToUpload2']['type'] != 'image/png') {
		$error = 'PNG ONLY';
		}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$filename="pic-icon2-".$myRandNew."-".$_REQUEST['myID'];
		...
		$update="";
		$update[]=$mod_tb_root_group."_pic2  	='".$picname."'";
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUploadSite(\'deletePicG2.php\',\'boxPic2\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
