...
<? if(is_file($valPic)){?>
  ...
  <? }?>
				...
				<!--  <table style="display:none;" width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxViewBorder ">
				 ...
				 <? if(is_file($valPic2)){?>
				 		<img src="<?=$valPic2?>"  style="float:left;border:#c8c7cc solid 1px;max-width:600px;"  />
				 		<div style="width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;" onclick="delPicUploadSite('deletePicG2.php','boxPic2')"  title="Delete file" ><img src="../img/btn/delete.png" width="22" height="22"  border="0"/></div>
				   <input type="hidden" name="picname2" id="picname2" value="<?=$valPicName2?>" />
				   <? }?>
				 </div></td>
				   </tr>
...
function ajaxFileUpload2(){
	...
	jQuery.ajaxFileUpload({
			url:'loadUpdatePicG2.php?myID=<?=$_POST["valEditID"]?>&masterkey=<?=$_REQUEST['masterkey']?>&langt=<?=$_REQUEST['inputLt']?>&delpicname='+valuepicname+'&menuid=<?=$_REQUEST['menukeyid']?>',
			secureuri:false,
