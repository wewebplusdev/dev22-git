...
<?php
	...
	if(!empty($_FILES['fileToUpload']['error'])){
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			$filename="pic-".$myRandNew."-".$_REQUEST['myID'];
		...
		$update=array();
		$update[]=$mod_tb_setting."_pic  	='".$picname."'";
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload(\'deletePicS.php\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
