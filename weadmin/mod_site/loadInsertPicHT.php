...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadHT';
	if(!empty($_FILES['fileToUploadHT']['error'])){
		switch($_FILES['fileToUploadHT']['error']){
	...
	}elseif($_FILES['fileToUploadHT']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$inputGallery=$_FILES['fileToUploadHT']['tmp_name'];
			$arrfile=$_FILES['fileToUploadHT'];
			$ERROR=$_FILES['fileToUploadHT']['error'];
			$TIME=time();
			if(!$ERROR) {
			$filename="pic-HT-".randomNameUpdate();
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUploadSite(\'deletePicInsertHT.php\',\'boxPicHT\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picHT\" type=\"hidden\" id=\"picHT\" value=\"$picname\" /> ";
