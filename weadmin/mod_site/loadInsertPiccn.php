...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadcn';
	if(!empty($_FILES['fileToUploadcn']['error'])){
		switch($_FILES['fileToUploadcn']['error']){
	...
	}elseif($_FILES['fileToUploadcn']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			if(file_exists($mod_path_office."/".$_REQUEST['delpicnameen'])) {
			...
			$inputGallery=$_FILES['fileToUploadcn']['tmp_name'];
			$arrfile=$_FILES['fileToUploadcn'];
			$ERROR=$_FILES['fileToUploadcn']['error'];

			$TIME=time();
			if(!$ERROR) {
			$filename="pic-".randomNameUpdate();
		...
		$msg .= "<img src=\"".$mod_path_pictures."/".$picname."\"  style=\"float:left;border:#c8c7cc solid 1px; width: calc(50% - 20px);\"  />";
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload_mul(\'deletePicInsertcn.php\',\'#boxPicNewcn\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnamecn\" type=\"hidden\" id=\"picnamecn\" value=\"$picname\" /> ";
