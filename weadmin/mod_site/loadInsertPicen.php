...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploaden';
	if(!empty($_FILES['fileToUploaden']['error'])){
		switch($_FILES['fileToUploaden']['error']){
	...
	}elseif($_FILES['fileToUploaden']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			$inputGallery=$_FILES['fileToUploaden']['tmp_name'];
			$arrfile=$_FILES['fileToUploaden'];
			$ERROR=$_FILES['fileToUploaden']['error'];
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload_mul(\'deletePicInserten.php\',\'#boxPicNewen\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnameen\" type=\"hidden\" id=\"picnameen\" value=\"$picname\" /> ";
