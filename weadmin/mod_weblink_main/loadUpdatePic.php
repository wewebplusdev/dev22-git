...
if (!empty($_FILES['fileToUpload']['error'])) {
    ...
    $TIME = time();
    if (!$ERROR) {
        if($_REQUEST['langt'] == "Eng"){
            $filename = "picen-" . $_REQUEST['myID']."-".randomNameUpdate(1);
        }else{
            $filename = "pic-" . $_REQUEST['myID']."-".randomNameUpdate(1);
        }
        // $myRandNew = rand(111, 999);
        // $filename = "pic$mylang-" . $_REQUEST['myID'] . "-" . randomNameUpdate(2);
