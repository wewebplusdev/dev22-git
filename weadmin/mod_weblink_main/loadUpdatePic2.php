...
if (!empty($_FILES['fileToUpload2']['error'])) {
    ...
    if (file_exists($mod_path_office . "/" . $_REQUEST['delpicname2'])) {
        @unlink($mod_path_office . "/" . $_REQUEST['delpicname2']);
    }

    if (file_exists($mod_path_real . "/" . $_REQUEST['delpicname2'])) {
        @unlink($mod_path_real . "/" . $_REQUEST['delpicname2']);
    }

    if (file_exists($mod_path_pictures . "/" . $_REQUEST['delpicname2'])) {
        @unlink($mod_path_pictures . "/" . $_REQUEST['delpicname2']);
    }
    ...
    if (!$ERROR) {
        ...
        $p->chktypepic();
        $ext = $p->ret();
        if ($ext == 'JPG' || $ext == "jpg" || $ext == "gif" || $ext == "GIF" || $ext == "png" || $ext == "PNG" || $ext == "svg" || $ext == "SVG") {
            ...
            if ($_REQUEST['langt'] == "Thai") {
                $update[] = $mod_tb_root . "_pic2  	='" . $picname . "'";
            ...
            $msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload2(\'deletePic2.php\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
            $msg .= "<input name=\"picname2\" type=\"hidden\" id=\"picname2\" value=\"$picname\" /> ";
        } else {
            $msg .= "<font color =red><b>กรุณาตรวจสอบประเภทไฟล์รูปภาพ อัพโหลดเฉพาะไฟล์ .jpg ,.gif ,.svg และ .png เท่านั้น</b></font>";
