...
<?php
	...
	// 	$error = 'PNG ONLY';
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$TIME=time();
			if(!$ERROR) {
			$filename="pic-icon-".$_REQUEST['myID'];
			...
			$ext=$p->ret();
			$ext=$p->ret(); 
			if($ext=='png' || $ext=='PNG' || $ext=='jpg' || $ext=='JPG' || $ext=='gif' || $ext=='GIF' || $ext=='svg' || $ext=='SVG'){
			...
			if($arrImgInfo[0]<=($sizeWidthPicG+10)){
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload(\'deletePicInsertG.php\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
