...
<?php
	...
	if(!empty($_FILES['fileToUpload2']['error'])){
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }  
			...
			$filename="pic2-".$myRandNew."-".$_REQUEST['myID'];
			...
			if($arrImgInfo[0]<=($sizeWidthIcon+10)){
		...
		$msg.="<font color =red><b>กรุณาตรวจสอบประเภทไฟล์รูปภาพ อัพโหลดเฉพาะไฟล์ .jpg ,.gif และ .png เท่านั้น</b></font>";
	}
