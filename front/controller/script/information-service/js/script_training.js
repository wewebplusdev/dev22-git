...
$(document).on('click','.clickdel_training',function () {
    ...
    if (CheckAction == 'adds') {
        ...
        let addPara = 'add';
        addcount_train(addPara);
    } else {
        $(this).closest('#clone_training').remove();
        let delPara = 'del';
        addcount_train(delPara);
    }
});

function addcount_train(action) {
    $('.training #clone_training').each(function(index,elem) {
        ...
        if (action == 'add') {
            ...
            // required inputs
            $(this).find('input[name="training['+(count)+'][]"]').attr('required', "required");
            $(this).find('select[name="training['+(count)+'][]"]').attr('required', "required");
        ...
        if (action == 'del') {
            ...
            //select2
            $(this).find('select[name="training['+(count+1)+'][]"]').attr('name','training['+(count)+'][]');
            $(this).find('select.select-control').select2({minimumResultsForSearch: -1}).end();//select2 after appendTo
