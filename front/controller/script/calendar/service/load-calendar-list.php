...
if (!empty($_REQUEST['myCalendarDatePer'])) {
    ...
    $txt_calendarList = $lang["calendar"]["activity"] . " " . $dateList[2]. " " . $date_array[$url->pagelang[2]][$dateList[1]] . " " . getYearLang($dateList[0], $url->segment[2]);
}else{
    $txt_calendarList = $lang["calendar"]["activityM"] . " " . $date_array[$url->pagelang[2]][$_REQUEST['myCalendarDate_Month']] . " " . getYearLang($_REQUEST['myCalendarDate_Year'], $url->segment[2]);
}
