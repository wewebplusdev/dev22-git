				...
				<div class="col-auto">
					<div class="brand">
						<a href="{$ul}/home" class="link" title="Gem and Jewelry Institute of Thailand">
							<img src="{$template}/assets/img/static/git-brand-2.svg"
								alt="Gem and Jewelry Institute of Thailand">
						</a>
					</div>
				</div>
				<div class="col">
				...
				<div class="col-auto">
					<div class="brand d-lg-none d-block">
						<a href="index-theme-2.php" class="link" title="Gem and Jewelry Institute of Thailand">
							<img src="{$template}/assets/img/static/git-brand-theme-2.svg"
								alt="Gem and Jewelry Institute of Thailand">
						</a>
					</div>
				</div>
						...
						<ul class="nav-list level-I">
							...
							{foreach $arrSitemap as $keyarrSitemap => $valuearrSitemap}
								...
								{if count($valuearrSitemap['list']) > 0}
									<li
										class="{if $keyarrSitemap eq 0}dropright{/if} {if $keyarrSitemap eq count($arrSitemap)}dropleft{/if} {if $segment eq $menuSegment[0]}active{/if}">
										<a href="javascript:void(0)" class="link link-submenu" data-link="{$keyarrSitemap}-menu"
											title="{$valuearrSitemap['group']['subject']}" data-toggle="dropdown"
								...
								{else}
									<li class="{if $segment eq $menuSegment[0]}active{/if}">
										<a {if $valuearrSitemap['group']['url'] neq "" && $valuearrSitemap['group']['url'] neq "#"}href="{$valuearrSitemap['group']['url']}"
												{if $valuearrSitemap['group']['target'] eq 2}target="_blank" {/if}
											{else}href="javascript:void(0);" 
											{/if} class="link link-submenu"
											data-link="{$keyarrSitemap}-menu" title="{$valuearrSitemap['group']['subject']}"
							...
							{foreach $valuearrSitemap['list'] as $keySubmenu => $valueSubmenu}
								{if count($valueSubmenu['menu']) > 0}
									<li class="dropdown-item">
										<a {if $valueSubmenu['subgroup']['url'] neq "" && $valueSubmenu['subgroup']['url'] neq "#"}href="{$valueSubmenu['subgroup']['url']}"
										...
										<ul class="item-list fluid bullet">
											{foreach $valueSubmenu['menu'] as $keyMenuLv3 => $valueMenuLv3}
												<li>
													<a {if $valueMenuLv3['url'] neq "" && $valueMenuLv3['url'] neq "#"}href="{$valueMenuLv3['url']}"
